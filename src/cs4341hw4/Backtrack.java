...
public class Backtrack {
	...
	private Item mrv(Map<Item, Bag> assignments){
		...
		for(Item i : unassigned){
			int availableBags = availableBags(i, assignments).size();
	...
	//Get available bags for an item
	private Collection<Bag> availableBags(Item i, Map<Item, Bag> assignments){
		Set<Item> assigned = assignments.keySet();
	...
	Collection<Bag> lcv(Item var, Map<Item, Bag> assignments){
		...
		for(Bag b : Main.bags){
			...
			b.contains.add(var);
			for(Item i : unassigned){
				constraints.put(b, constraints.get(b) + Main.bags.size() - availableBags(i, assignments).size());
			}
			b.contains.remove(var);
